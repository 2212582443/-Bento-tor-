...
class Session():
    ...
    def execute(self, exec_data):
        ...
        self.function_proc= subprocess.Popen(cmd, stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.PIPE)#就是这个子进程没有输出
    ...
    def read_output(self):
        ...
        datalen= self.read_handle.read(8)#这个地方有问题，说明将结果存储的时候出问题了
        if len(datalen) == 8:
...
def exchange(session: Session):
    ...
    """
    proc_out= session.function_proc.stdout#这里没有输出
    ...
    while True:
        ...
        if len(bdata) == 8:
            ...
            write_handle.flush()
            logging.info(f"({session.session_id}) wrote output")#这里就没有执行，proc_out说明根本没有读到数据
        else:
